        ...
        for i in range(nbsize):
            ...
            for co in range(c_out):
                for ci in range(c_in):
                    output[out_index,
                           co] += feats[in_index, ci] * weights[k, ci, co]
            nbaddr += 1
    if precompute:
...
def test_spconv():
    ...
    for i, file in enumerate(file_list):
        ...
        kernel = torch.rand((k_vol, in_channel, out_channel),
                            dtype=Dtype,
                            device='cuda')
        ...
        _ = dev_output.clone().cpu()
        _ = out_nnz * out_channel
        ...
        # (output, output_cpu, rtol=1e-2, atol=1e-4) == True
