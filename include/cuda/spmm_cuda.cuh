  ...
  for (; row < nr; row += stride)
  {
    ...
    if ((end - start) > 0)
    {
      for (Index p = start; p < end; p++)
      {
        ...
        if ((REDUCE::Op == MAX && (res < val_pre_red)) || ((REDUCE::Op == MIN) && (res > val_pre_red)))
        {
          E_k_idx = col;
        }
...
//   to compute SpMM.
template <typename Index, typename DType, typename access_t, typename REDUCE, typename COMPUTE>
__global__ void csrspmm_parreduce_rowbalance_kernel(
    const Index nr, const Index feature_size, const Index rowPtr[],
    const Index colIdx[], const DType values[], const DType dnInput[], DType dnOutput[], Index E[])
{
...
#pragma unroll
      for (Index i = 0; i < CoarsenFactor; i++)
      {
        ...
        if ((REDUCE::Op == REDUCEOP::MAX && val_pre_red > c[i]) || (REDUCE::Op == REDUCEOP::MIN && val_pre_red < c[i]))
        {
          c[i] = val_pre_red;
          E_k_idx[i] = k;
        }
        else if (REDUCE::Op == REDUCEOP::SUM || REDUCE::Op == REDUCEOP::MEAN)
        {
          c[i] += val_pre_red;
        }
      }
...
__global__ void
csrspmm_seqreduce_rowbalance_with_mask_kernel(const Index nr, const Index feature_size,
                                              const Index rowPtr[], const Index colIdx[],
                                              const DType values[], const DType dnInput[], const Index E[],
                                              DType dnOutput[])
{
  Index row_tile = blockDim.y; // 8
  Index subwarp_id = threadIdx.y;
