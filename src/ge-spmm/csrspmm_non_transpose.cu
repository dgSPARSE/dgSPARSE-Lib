...
__global__ void csrspmm_non_transpose_parreduce_rowbalance_kernel(const int M,
                                                                const int N,
                                                                const int K,
                                                                ...
                                                                float C[])
{
// RFC: Implementing Sparse matrix-vector produce on throughput-oriented processors, SC2009
...
template<int CoarsenFactor>
__global__ void csrspmm_non_transpose_parreduce_nnzbalance_kernel(const int M,
                                                                const int N,
                                                                const int K,
                                                                const int nnz_,
                                                                const int csr_indptr[],
...
Ndim_Residue:
    ...
    if (row_intv==0) {
        ...
        if (lane_id==0) 
        {
            #pragma unroll
            for (int i=0; i<CoarsenFactor; i++) {
                if (i < valid_lane_num) {atomicAdd(C_panels[i] + row, c[i]);}
            }            
        }
    }
    else {
...
template<int CoarsenFactor>
__global__ void csrspmm_non_transpose_seqreduce_rowbalance_kernel(const int M,
                                                                const int N,
                                                                const int K,
                                                                const int csr_indptr[],
                                                                const int csr_indices[],
...
template<int CoarsenFactor>
__global__ void csrspmm_non_transpose_seqreduce_nnzbalance_kernel(const int M,
                                                                const int N,
                                                                const int K,
                                                                const int nnz_,
                                                                const int csr_indptr[],
                                                ...
                                                )
{
    ...
    if (coarsen_factor == 4) {
    csrspmm_non_transpose_parreduce_rowbalance_kernel<4><<<gridDim, blockDim>>>(spmatA.nrow, 
                                                N,
                                                spmatA.ncol,
                                                spmatA.indptr,
                                                spmatA.indices,
    ...
    csrspmm_non_transpose_parreduce_rowbalance_kernel<2><<<gridDim, blockDim>>>(spmatA.nrow, 
                                                N,
                                                spmatA.ncol,
                                                spmatA.indptr,
                                                spmatA.indices,
    ...
    csrspmm_non_transpose_parreduce_rowbalance_kernel<1><<<gridDim, blockDim>>>(spmatA.nrow, 
                                                N,
                                                spmatA.ncol,
                                                spmatA.indptr,
                                                spmatA.indices,
                                                ...
                                                )
{
    ...
    if (coarsen_factor == 4) {
    csrspmm_non_transpose_parreduce_nnzbalance_kernel<4><<<gridDim, blockDim>>>(spmatA.nrow, 
                                                N,
                                                spmatA.ncol,
                                                spmatA.nnz,
                                                spmatA.indptr,
    ...
    csrspmm_non_transpose_parreduce_nnzbalance_kernel<2><<<gridDim, blockDim>>>(spmatA.nrow, 
                                                N,
                                                spmatA.ncol,
                                                spmatA.nnz,
                                                spmatA.indptr,
    ...
    csrspmm_non_transpose_parreduce_nnzbalance_kernel<1><<<gridDim, blockDim>>>(spmatA.nrow, 
                                                N,
                                                spmatA.ncol,
                                                spmatA.nnz,
                                                spmatA.indptr,
                                                ...
                                                )
{
    ...
    if (coarsen_factor == 4) {
    csrspmm_non_transpose_seqreduce_rowbalance_kernel<4><<<gridDim, blockDim>>>(spmatA.nrow, 
                                    N,
                                    spmatA.ncol,
                                    spmatA.indptr,
                                    spmatA.indices,
    ...
    csrspmm_non_transpose_seqreduce_rowbalance_kernel<2><<<gridDim, blockDim>>>(spmatA.nrow, 
                                    N,
                                    spmatA.ncol,
                                    spmatA.indptr,
                                    spmatA.indices,
    ...
    csrspmm_non_transpose_seqreduce_rowbalance_kernel<1><<<gridDim, blockDim>>>(spmatA.nrow, 
                                    N,
                                    spmatA.ncol,
                                    spmatA.indptr,
                                    spmatA.indices,
                                                ...
                                                )
{
    ...
    if (coarsen_factor == 4) {
    csrspmm_non_transpose_seqreduce_nnzbalance_kernel<4><<<gridDim, blockDim>>>(spmatA.nrow, 
                                    N,
                                    spmatA.ncol,
                                    spmatA.nnz,
                                    spmatA.indptr,
    ...
    csrspmm_non_transpose_seqreduce_nnzbalance_kernel<2><<<gridDim, blockDim>>>(spmatA.nrow, 
                                    N,
                                    spmatA.ncol,
                                    spmatA.nnz,
                                    spmatA.indptr,
    ...
    csrspmm_non_transpose_seqreduce_nnzbalance_kernel<1><<<gridDim, blockDim>>>(spmatA.nrow, 
                                    N,
                                    spmatA.ncol,
                                    spmatA.nnz,
                                    spmatA.indptr,
