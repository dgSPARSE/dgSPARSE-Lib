...
at::Tensor sparse_mapping(
  ...
  /********************************************************************/
  // default stream
  at::Tensor index = -torch::ones(
      {table_size}, at::device(in_coords.device()).dtype(at::ScalarType::Int));
  at::Tensor value = torch::zeros(
      {table_size}, at::device(in_coords.device()).dtype(at::ScalarType::Long));
  ...
  if (separate_mid) {
    ...
    if ((l_stride_x == 1 || l_stride_x == k_size_x) &&
      ...
      at::Tensor ocoords_code_space = torch::zeros(
          {in_nnz}, at::device(in_coords.device()).dtype(at::ScalarType::Long));

      int64_t *ocoords_code_ptr = ocoords_code_space.data_ptr<int64_t>();

      coordsDownsample<3259><<<CEIL(in_nnz, 16), dim3(16, 1, 1), 0, 0>>>(
          in_nnz, stride_x, stride_y, stride_z, in_coords_ptr,
          ocoords_code_ptr);

      // in defaut order: b -> x -> y -> z
      thrust::sort(thrust::cuda::par.on(0), ocoords_code_ptr,
                   ocoords_code_ptr + in_nnz);

      int64_t *new_end = thrust::unique(
          thrust::cuda::par.on(0), ocoords_code_ptr, ocoords_code_ptr + in_nnz);
      ...
      at::Tensor ocoords_code_space = -torch::ones(
          {in_nnz * k_vol},
      ...
      coordsDownsampleExpand<3259, 4><<<CEIL(in_nnz, 4), dim3(4, 4, 1), 0, 0>>>(
          in_nnz, k_vol, k_size_x, k_size_y, k_size_z, t_stride_x, t_stride_y,
          t_stride_z, l_stride_x, l_stride_y, l_stride_z, padding_ptr, min_ptr,
          max_ptr, in_coords_ptr, ocoords_code_ptr);

      // extract the valid output coords code
      int64_t *valid_end =
          thrust::remove(thrust::cuda::par.on(0), ocoords_code_ptr,
                         ocoords_code_ptr + (in_nnz * k_vol), -1);

      int valid_num = valid_end - ocoords_code_ptr;

      // in defaut order: b -> x -> y -> z
      thrust::sort(thrust::cuda::par.on(0), ocoords_code_ptr,
                   ocoords_code_ptr + valid_num);

      int64_t *new_end =
          thrust::unique(thrust::cuda::par.on(0), ocoords_code_ptr,
                         ocoords_code_ptr + valid_num);
  ...
  if (separate_mid) {
    _queryhash_subm<16, 4><<<CEIL(out_nnz, 16), dim3(4, 16), 0, 0>>>(
        in_nnz, out_nnz, table_size, out_coords_ptr, k_size_x, k_size_y,
        k_size_z, k_vol, value_ptr, index_ptr, map_ptr, knnz_ptr, separate_mid);
  } else {
    _queryhash_sp<16, 4><<<CEIL(out_nnz, 16), dim3(4, 16), 0, 0>>>(
        in_nnz, out_nnz, table_size, out_coords_ptr, k_size_x, k_size_y,
        k_size_z, k_vol, l_stride_x, l_stride_y, l_stride_z, padding_ptr,
