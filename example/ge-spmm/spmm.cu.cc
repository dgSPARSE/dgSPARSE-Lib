...
#include <vector>     
#include <gespmm.h>            // gespmmCsrSpMM()
...
int main(int argc, const char** argv) {
    ...
    if (correct) {
        ...
        printf("[Cusparse] Report: spmm A(%d x %d) * B(%d x %d) sparsity %f (nnz=%d) \n Time %f (ms), Throughput %f (gflops).\n", 
            M, K, K, N, 
            (float)nnz / M / K,
            nnz,
            kernel_dur_msecs,
    ...
    for (auto alg: algs) {
        ...
        if (correct) {
            ...
            printf("[GE-SpMM][Alg: %d] Report: spmm A(%d x %d) * B(%d x %d) sparsity %f (nnz=%d) \n Time %f (ms), Throughput %f (gflops).\n", 
                alg,
                M, K, K, N, 
                (float)nnz / M / K,
                nnz,
